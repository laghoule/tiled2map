...
- **`<prefix>-scene.inc`** — An assembly include file declaring the map buffers (`world_bg_buffer`, `world_fg_buffer`) and describing each scene with its neighbors and music reference.
...
The file declares two map buffers (`world_bg_buffer` and `world_fg_buffer`), each sized to hold all tiles of the entire map plus a 2-byte header. It then defines one `SCENE` entry per scene, with references to the shared map offset, music, and neighboring scenes (North, South, East, West). A value of `0` means no neighbor in that direction.

```asm
; GENERATED BY tiled2map (https://github.com/laghoule/tiled2map)
; DO NOT EDIT

world_bg_buffer DB 882 DUP(0)
world_fg_buffer DB 882 DUP(0)

; --- Scene # 0 ---
SCENE_0_0 SCENE < \
  OFFSET world_bg_buffer + 2, \
  OFFSET world_fg_buffer + 2, \
  OFFSET MUSIC_0_0, \
  0, \
  OFFSET SCENE_0_1, \
  OFFSET SCENE_1_0, \
