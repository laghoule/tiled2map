...
| ------ | ------- | ---------------------- |
| 0+     | N bytes | Raw indexed pixel data |

The file contains no header. Tiles are stored sequentially, one after another, sorted by GID order. Tile dimensions and count are declared as ASM constants in the `-refs.inc` file (`TILE_WIDTH`, `TILE_HEIGHT`, `TILES_COUNT`).

### `.map` — Binary Map Data

| Offset | Size    | Description            |
| ------ | ------- | ---------------------- |
| 0+     | N bytes | Local tile IDs (uint8) |

The file contains no header. Both layers are merged: background tiles first, followed by foreground tiles. Map data is organized **scene by scene** (left-to-right, top-to-bottom). Within each scene, tiles are stored row by row. The local tile ID is the index of the tile in the master atlas (0-based, sorted by GID). Map dimensions and layer size are declared as ASM constants in the `-scne.inc` file (`LAYER_WIDTH`, `LAYER_HEIGHT`, `LAYER_SIZE`).
...
master_tileset_buffer DB 768 DUP(0)
MASTER_TILE_WIDTH EQU 16
...
The buffer size formula is: `tileWidth × tileHeight × tileCount` (no header).

### `-scne.inc` — Assembly Scene Descriptors

The file declares a single merged map buffer (`<prefix>_buffer`) sized to hold all tiles of the entire map for **both** background and foreground layers. It also declares ASM constants for the map dimensions and layer size, and defines one `SCENE` entry per scene with references to the shared map offset and neighboring scenes (North, South, East, West). A value of `0` means no neighbor in that direction. Buffer and label names are lowercase and prefixed with the file prefix; constants are uppercase.

```asm
; GENERATED BY tiled2map (https://github.com/laghoule/tiled2map)
; DO NOT EDIT

master_buffer DB 1760 DUP(0)
