...
The file declares two map buffers (`<prefix>_bg_buffer` and `<prefix>_fg_buffer`), each sized to hold all tiles of the entire map plus a 2-byte header. It then defines one `SCENE` entry per scene, with references to the shared map offset and neighboring scenes (North, South, East, West). A value of `0` means no neighbor in that direction. All symbol names are lowercase and prefixed with the file prefix.

```asm
; GENERATED BY tiled2map (https://github.com/laghoule/tiled2map)
; DO NOT EDIT

master_bg_buffer DB 882 DUP(0)
master_fg_buffer DB 882 DUP(0)

; --- Scene # 0 ---
master_scene_0_0 SCENE <OFFSET master_bg_buffer + 2, OFFSET master_fg_buffer + 2, 0, OFFSET master_scene_0_1, OFFSET master_scene_1_0, 0>
```
